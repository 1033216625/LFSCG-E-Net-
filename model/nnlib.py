...
def conv_pad(h, n=64, s=1, k=3):
    ...
    h = tf.pad(
        h,
        tf.constant([[0, 0], [padsz, padsz], [padsz, padsz], [0, 0]]),#batch，h,w,channels
        mode='SYMMETRIC')
...
# 这段代码定义了一个残差块 resblk，让我们逐行解释它：

#     resblk = [resi, [[conv, nch_res], [runit], [conv, nch_res]]]：定义了一个列表 resblk，包含两个元素。

#     resi：作为第一个元素，是一个函数，表示使用之前解释的 resi 函数作为残差块的主体结构。

#     [[conv, nch_res], [runit], [conv, nch_res]]：作为第二个元素，是一个列表，表示残差块的子层序列。其中每个子列表表示一个子层操作：
#         [conv, nch_res]：一个卷积操作，使用 conv 函数，nch_res 是卷积核的通道数。
#         [runit]：一个 ReLU 激活函数操作，表示使用 runit 函数。
#         [conv, nch_res]：另一个卷积操作，使用 conv 函数，卷积核通道数也是 nch_res。

# 这样定义的 resblk 可以在神经网络的构建过程中重复使用，方便了残差块的构建。


#自己写的se注意力模块
def se_attention(input_tensor, reduction_ratio=16, name='se_attention'):
    with tf.variable_scope(name):
        channels = input_tensor.get_shape().as_list()[-1]
        squeeze = tf.reduce_mean(input_tensor, axis=[1, 2], keepdims=True)
        excitation = tf.layers.dense(squeeze, units=channels // reduction_ratio, activation=tf.nn.relu)
        excitation = tf.layers.dense(excitation, units=channels, activation=tf.nn.sigmoid)
        scaled_output = input_tensor * excitation
        return scaled_output


def NN(name, layers, output_layers = False):#(name 是模型的名称，layers 是神经网络的层列表，output_layers 用于指示是否返回所有中间层的输出)
    ilayer = 0
    h = layers[0]   #ilayer 用于追踪当前层的索引，h 用于存储当前层的输出，初始值设为输入层的输出。intermediate 是一个空列表，用于存储每一层的输出。
    intermediate = []
    with tf.variable_scope(name, reuse=False) as scope:#创建了一个 TensorFlow 变量作用域，命名为 name。reuse=False 表示在这个作用域内的变量不会被重用。
        for layer in layers[1:]:        #遍历了除了输入层之外的所有层
            with tf.variable_scope('{}'.format(ilayer)):#在每一层内部，创建了一个 TensorFlow 变量作用域，名字是当前层的索引号，用于组织当前层的变量。
                h = layer[0](h, *layer[1:])  #调用当前层的函数，传入当前层的输入 h 以及其他参数 *layer[1:]，并将计算结果保存在 h 中
                intermediate.append(h)  #当前层的输出 h 添加到 intermediate 列表中
            ilayer += 1    #更新当前层的索引号
    if output_layers:
        return h, intermediate  #如果为 True，则返回最后一层的输出 h 和所有中间层的输出 intermediate
    else:
        return h
