...
def create_canonical_coordinates(batch_size, img_size, anchor_res):  #2,160,20
        block_size = img_size // anchor_res  #8
        anchor_offset = block_size // 2   #4
        # block center locations
        xx = np.linspace(anchor_offset, img_size - anchor_offset, anchor_res, dtype = np.float32)  #（4,160-4,20）
        # yy = np.linspace(anchor_offset, h - anchor_offset, anchor_res, dtype = np.float32)
        print('coordinates: ', xx)
        #coordinates:  [  4.  12.  20.  28.  36.  44.  52.  60.  68.  76.  84.  92. 100. 108.116. 124. 132. 140. 148. 156.]
        xv, yv = np.meshgrid(xx, xx)
        
        #xv = [[  4.  12.  20.  28.  36.  44.  52.  60.  68.  76.  84.  92. 100. 108. 116. 124. 132. 140. 148. 156.]
        #[  4.  12.  20.  28.  36.  44.  52.  60.  68.  76.  84.  92. 100. 108. 116. 124. 132. 140. 148. 156.]
        #...
        #[  4.  12.  20.  28.  36.  44.  52.  60.  68.  76.  84.  92. 100. 108. 116. 124. 132. 140. 148. 156.]]
      
    #   yv = [[  4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.   4.]
...
# 这行代码将 xv 和 yv 的坐标值连接成一个二维数组 canonical_coords，其中每一行表示一个网格点的坐标，第一列是 y 坐标，第二列是 x 坐标。       
#  canonical_coords = [[  4.   4.]
                     # [ 12.   4.]
                     # [ 20.   4.]
                     # [ 28.   4.]
                     # [ 36.   4.]
...
#这行代码使用 TensorFlow 的 tile 函数来创建一个形状为 [batch_size, height*width, 2] 的张量，其中 batch_size 是批大小，height*width 是网格点数量，2 表示每个网格点的两个坐标。这个张量是通过重复 t_canonical_coords 批量大小 batch_size 次得到的
###################################################################################################
...
def oper_img2prog_final(t_img, params = dict(), name='img2prog'):
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!t_act1=%s' % t_act1)
        t_act2 = NN('feat2feat',
            [t_act1, *[rblk for _ in range(rblk_num)]])
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!t_act2=%s' % t_act2)
        t_out  = NN('feat2prog',
...
def translate_tensor(h, dx, dy):#对instruction进行位移
    h = tf.pad(#对h进行填充
        h,
        tf.constant([[0, 0], [1, 1], [1, 1]]),#第一维不填充，第二维第三维两侧各填充1单元格
        mode='SYMMETRIC')#对称填充模式
    imsz = tf.shape(h)
    return h[:,(1+dy):(imsz[1]+dy-1),(1+dx):(imsz[2]+dx-1)]#对h进行切片操作，以移动张量内容，实现平移效果。第一维全取不切片，第二维从垂直方向切片，从1+dy开始，到imsz[1]+dy-1 表示终止索引（imsz[1] 是填充后的张量在垂直方向上的尺寸）
...
def tf_MILloss_xentropy(labels, logits, weight=None):
    ...
    for i in range(len(dx)):    # 8 directions
        ...
        if weight is not None:#如果给定了权重 weight，则将其与交叉熵相乘，这是为了根据权重调整损失。
        ...
        if i == 0: # make it prefer the first element
            xentropy -= 1e-6      #对第一个方向的交叉熵损失减去了一个非常小的值 1e-6，以确保它在最小化操作中具有更高的优先级。
    ...
    entropy_tensor = tf.stack(lst_entropy, axis=3)[:,1:-1,1:-1,:] # BxHxWxD  #它将每个方向上的交叉熵损失堆叠在一起
    print('xentropy dim: {}'.format(entropy_tensor.get_shape()))
    entropy_disp = tf.reduce_mean(entropy_tensor,axis=[1,2], keepdims=True) # spatial collapsing  #是对空间维度（高度和宽度）进行平均的张量，这样可以将空间信息折叠起来。
    ...
    total_entropy = tf.reduce_min(entropy_disp, axis=3, keepdims=True) # take minimum loss over displacement  #对位移方向进行最小化操作得到的张量，它表示了最终的损失。
