...
class FeedForwardNetworks(Model):
    ...
    def model_define(self,
        ...
        if self.oparam.params.get('lfscgenet', 0) == 1: # complex net
            # if 'tran' in X_in.keys():
            #     del X_in['tran']
            net = lfscgenet.model_composited_RFI_complexnet(X_in, Y_out, self.oparam.params)
            if is_train:
                loss_dict_Disc, loss_dict_Gene, metrics = lfscgenet.total_loss_RFI(
    ...
    def build_model(self):
        ...
        # pdb.set_trace()  # check whether unsup eta structure is good enough
    ...
    def train(self):
        ...
        if 1: # just for now 'bn' in runit_type or 'in' in runit_type:
            base_deps.extend(tf.get_collection(tf.GraphKeys.UPDATE_OPS))         #保证批量归一化能正确更新
        else:
            base_deps = None
