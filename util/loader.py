...
def read_image(fname, expand = True, aug_scale = True):
    ...
    img = cv2.imread(fname, cv2.IMREAD_COLOR)        #以彩色图像加载
    if img is None:
        print('Image %s not found!' % fname)
    img = np.squeeze(img[:,:,choice(3)])             #，随机选择一个通道输出，转换为灰色图像

    # img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # to grayscale
    img = img.astype(np.float32) / 255. # BGR format              #归一化
    if aug_scale:                                       #是否进行随机缩放，这里是1
        
        randscale = 0.2*(np.random.rand(1)-0.6)         #缩放因子
        img = np.minimum(img*(1.+randscale), 1.)         #图像乘缩放因子，范围限制在0-1
        img = img.astype(np.float32)

    # cv2.resize(img, (0, 0), fx=scale, fy=scale)
    if expand:
        return np.expand_dims(img, axis=2) # we need three dimensions (not two) even though grayscale         #增加图像维度
    else:
        return img
