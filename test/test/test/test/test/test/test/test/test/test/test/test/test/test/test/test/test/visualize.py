...
if __name__ == "__main__":
    ...
    for i in range(1, len(sys.argv)):
        ...
        try:
            pattern = readInstructionsPNG(fname)
            image = KnittingViz.getInstance().printInstructionsNice(pattern)
            outname = os.path.splitext(os.path.basename(fname))[0] + '_viz.png'
            full_outname = os.path.join(save_dir, outname)
            cv2.imwrite(full_outname, image, [cv2.IMWRITE_JPEG_QUALITY, 95])
            print('Saved %s' % full_outname)
