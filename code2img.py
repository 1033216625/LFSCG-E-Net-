...
# flags.DEFINE_string("output_dir", "./bottleneckgaidong", "The output directory")
...
flags = tf.app.flags
flags.DEFINE_string("render_type", "dense", "The renderer type")
flags.DEFINE_string("output_dir", "./img", "The output directory")
flags.DEFINE_integer("batch_size", 1, "Number of images to process at once")
flags.DEFINE_float("noise_level", 0.45, "Noise level to apply on the instruction")
FLAGS = flags.FLAGS
...
def main(argv):
    ...
    with tf.Session(config = config) as sess:
        ...
        while batch_start < batch_end:
            ...
            names = []
            for i in range(batch_size):
                img_idx = min(batch_end - 1, batch_start + i)# img_idx = 0
                fname = inputs[img_idx]
                img = read_image(fname)
                batch.append(img)
                names.append(fname)
